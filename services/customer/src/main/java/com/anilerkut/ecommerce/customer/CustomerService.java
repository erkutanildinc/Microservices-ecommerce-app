package com.anilerkut.ecommerce.customer;

import com.anilerkut.ecommerce.exception.CustomerNotFoundException;
...
public class CustomerService {
    ...
    public void updateCustomer(CustomerRequest request) {
        var customer = repository.findById(request.id()).orElseThrow(() -> new CustomerNotFoundException(
                String.format("Cannout update Customer:: No customer found with the provided ID:: %s",request.id())
        ));
    ...
    public List<CustomerResponse> findAllCustomers() {
        return repository.findAll().stream().map(customerMapper::fromCustomer).collect(Collectors.toList());
    }
